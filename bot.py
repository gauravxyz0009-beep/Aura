...
def show_credits(user_id):
    ...
    return (
        "💳 My Credits\n\n"
        f"⭐ Available Credits: {data.get('credits', 0)}\n"
        f"📚 Lectures Watched: {data.get('lectures', 0)}"
    )
...
def study_tracker(user_id):
    ...
    return (
        "📊 Study Tracker\n\n"
        f"📚 Completed Lectures: {data.get('lectures', 0)}\n"
        f"🔥 Streak: {data.get('streak', 1)} Days\n"
            ...
            if "message" in update:
                ...
                if data == "physics":
                    send_message(
                        chat_id,
                        "📘 Physics Live Lec\n\nSelect Teacher 👇",
                        [[{"text": "⚡ MR Sir", "callback_data": "mr"}],
                         [{"text": "🔥 Saleem Sir", "callback_data": "saleem"}]]
                    )
                    ...
                    send_message(
                        ...
                        [[{"text": "Sudhanshu Sir", "callback_data": "sudhanshu"}],
                         [{"text": "Amit Sir", "callback_data": "amit"}]]
                    )
                    ...
                    send_message(
                        chat_id,
                        "🧬 Biology Live Lec\n\nSelect Option 👇",
                        [[{"text": "Biology Live 1", "callback_data": "bio1"}],
                         [{"text": "Biology Live 2", "callback_data": "bio2"}]]
                    )
                    ...
                    if claim_daily(chat_id):
                        ...
                        send_message(chat_id, "⏳ Already Claimed\n\nNext claim after 24 hours")
                ...
                # Admin Callbacks (Basic)
                    ...
                    elif data in ["admin_credit", "admin_link"]:
                        send_message(chat_id, "Admin Feature Coming Soon...")
